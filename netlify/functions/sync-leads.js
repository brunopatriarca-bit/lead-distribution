...
const STATE_TO_REGION = {
  ...
  AM:'NONE', PA:'NONE', AC:'NONE', RO:'NONE', RR:'NONE', AP:'NONE', TO:'NONE',
...
function extractCoords(row) {
  ...
  for (const f of ['latitude_fim','latitude_inicio','lat','latitude']) {
    if (row[f]!=null&&row[f]!==''&&!isNaN(parseFloat(row[f]))) { lat=parseFloat(row[f]); break; }
  }
  for (const f of ['longitude_fim','longitude_inicio','lon','longitude','lng']) {
    if (row[f]!=null&&row[f]!==''&&!isNaN(parseFloat(row[f]))) { lon=parseFloat(row[f]); break; }
  }
  return { lat, lon };
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    for (const row of rows) {
      const externalId = String(row.id ?? row.codigo ?? row.id_relatorio ?? JSON.stringify(row).slice(0,60));
    ...
    return { statusCode:200, headers, body:JSON.stringify({ success:true, fetched:rows.length, inserted, updated }) };
  } catch (err) {
    console.error(err);
