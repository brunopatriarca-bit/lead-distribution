...
exports.handler = async (event) => {
  ...
  try {
    ...
    // 1. Buscar visitas da Paytrack
    const rows = await fetchPaytrack();
    let inserted=0, updated=0, matched=0;

    // 2. Buscar leads Neoway geocodificados para cruzamento
    const neowayLeads = await sql`
      SELECT id, name, cnpj, lat, lon
      FROM neoway_leads
      WHERE lat IS NOT NULL AND lon IS NOT NULL
    `;

    // 3. Inserir/atualizar cada visita
    for (const row of rows) {
      const externalId = String(row.id??row.id_relatorio??row.codigo??JSON.stringify(row).slice(0,60));

      const { lat: latInicio, lon: lonInicio } = extractCoords(row);
      const stateCode  = coordsToState(latInicio, lonInicio);
      ...
      // Coordenadas do DESTINO da visita para cruzar com Neoway
      const latFim = parseFloat(row.latitude_fim||'0');
      const lonFim = parseFloat(row.longitude_fim||'0');

      // Cruzar destino com leads Neoway (raio 2 km)
      let destinationName = null;
      let destinationCnpj = null;
      if (latFim && lonFim && !isNaN(latFim) && !isNaN(lonFim) && latFim!==0) {
        for (const lead of neowayLeads) {
          const dist = haversine(latFim, lonFim, lead.lat, lead.lon);
          if (dist <= 2) {
            destinationName = lead.name;
            destinationCnpj = lead.cnpj;
            matched++;
            break;
          }
        }
      ...
      const result = await sql`
        ...
        VALUES (
          ${externalId}, ${JSON.stringify(row)}, ${stateCode}, ${regionCode},
          ${nomeColab}, ${destinationName}, NOW()
        )
    ...
    return {
      ...
      body:JSON.stringify({ success:true, fetched:rows.length, inserted, updated, matched }),
    };
